...
The agent uses a hybrid finite state machine (FSM) and LLM pattern. The FSM owns all hard rules and state transitions. The LLM handles language only: extracting structured data from user input. Response strings are hard-coded per state rather than generated by the LLM, which keeps them deterministic and policy-safe.
...
FSM: advance state if conditions met
    |
Python: return hard-coded response string for current state
    |
{"message": "..."}
```
